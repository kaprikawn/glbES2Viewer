...
MeshData populate_mesh_data( u32 target_mesh_index, const char* json_string, ReadFileResult* gltf_file, f32 scale ) {
  ...
  AccessorData colour0_accessor_data     = get_accessor_data( mesh_position_indices.colour_0, json_string, json_char_count );
  ...
  BufferViewData colour0_buffer_view_data      = get_buffer_view_data( colour0_accessor_data.buffer_view, json_string, json_char_count );
  ...
  u32 colour0_data_total_offset      = bin_start_offset + colour0_buffer_view_data.byte_offset;
  ...
  result.colour0_data_in_bytes     = colour0_buffer_view_data.byte_length;
  ...
  result.colour0_count             = colour0_accessor_data.count;
  ...
  result.colour0_data              = ( f32* )( ( char* )gltf_contents + colour0_data_total_offset );
...
GltfBufferViewInfo get_glft_buffer_view_info( u32 target_mesh_index, const char* gltf_contents, JsonString json, s32 content_type ) {
  ...
  if( content_type == GLTF_VERTICES ) {
    ...
    accessor_data = get_accessor_data( mesh_position_indices.colour_0, json_string, json_char_count );
  } else {
    return result;
  }
